...
private:
	...
	inline int clampAndConvert(int v);
};
...
int CColor::clampAndConvert(int v)
{
	if (v < 0)return v + 255;
...
{
	srand(time(0));
	pallete[0] = -2348062;// 0xff000000 | (rand()*rand()); //that constant is a good seed
	//cout << "color seed: " << pallete[0] << endl;
	int hue;
	int R, G, B;

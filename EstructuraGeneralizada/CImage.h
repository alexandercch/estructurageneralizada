...
public:
	...
	int m_nchannels;
	//load 2d  images
	CImage(string filename, bool is_3d = false) :m_layers(0), m_nchannels(0)
	{
		if (is_3d)
		{
		...
		else
		{
			...
			//cout << "number of channels" << m_imagen->spectrum() << " - " << m_nchannels << endl;
		}
	};
	CImage(int height, int width) :m_height(height), m_width(width), m_layers(0), m_nchannels(0)
	{
		m_imagen = new CImg<IType>(width, height,  1, 3);
		m_nchannels = 1;
	}
	CImage(int height, int width, int layers) :m_height(height), m_width(width), m_layers(layers), m_nchannels(0)
	{
		m_imagenes = new CImg<IType>*[layers];
		for (int i = 0; i < layers; ++i)
			m_imagenes[i] = new CImg<IType>(width, height, 1, 3);
		m_nchannels = 1;
	}
	virtual ~CImage() {};
	...
	unsigned int get_pixel(int row, int column)
	{
		...
		r = (int)(*m_imagen)(column, row, 0, 0);
		g = (int)(*m_imagen)(column, row, 0, 1 * m_nchannels);
		b = (int)(*m_imagen)(column, row, 0, 2 * m_nchannels);
		/*r = m_imagen->data()[pos];
	...
	void set_pixel(int row, int col, unsigned int pixel)
	{
		...
		(*m_imagen)(col, row, 1 * m_nchannels) = g;
		(*m_imagen)(col, row, 2 * m_nchannels) = b;
		//cout << "set pixel no problem!";
	}
	//set 3d image pixel
	...
	void display3d()
	{
		///the next 3 lines, are to fast debug 3d images, could cause error
		///dont forget to erase it ;)
		int middle_layer = m_layers / 2;
		m_imagenes[middle_layer]->display();
		return;

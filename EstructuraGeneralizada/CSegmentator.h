...
template<class G>
CSegmentator<G>::CSegmentator(G *_input) :input(_input), cntr(0), m_segmentation_difference(MAX_SEGMENTATION_DIFFERENCE)
{
};
...
void CSegmentator<G>::binary_segmentation()
{
	...
	{
		iter.setData(handle_color.process_pixel_binary(iter.getData()));
	}
}
...
void CSegmentator<G>::to_gray_scale()
{
	...
	for (iter = input->begin(); iter != input->end(); iter++)
	{
		iter.setData(handle_color.process_gray_scale(iter.getData()));
	}
}
	...
	for (iter = input->begin(); iter != input->end(); iter++)
	{
		...
		while (node_queue.size())
		{
			...
			for (int i = 0; i < input->m_number_of_neighbors; ++i)
			{
				...
				if (handle_color.rgb_difference(neighbor.getData(), actual.getData()) //(neighbor->m_data, actual->m_data)
		...
		//cout << "label: "<<current_label_value << ":"<<members<<'\t';
		//Sleep(1000);
...
template<class G>//this code is for debuging :3
